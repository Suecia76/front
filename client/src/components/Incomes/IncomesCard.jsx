...
const IncomesCard = ({ icon, iconName, title, categoria_fk, amount, state, stateClassName, date, _id }) => {
    ...
    <article key={_id} className="transaction-card">
        <a className='transaction-card__link' href={`/incomes/${_id}`}>
            ...
            <div className='transaction-card__info'>
                <h3 className='transaction-card__title'><span className='transaction-card__amount'>${amount}</span> - {title}</h3>
                <p className='transaction-card__category'><span>Categoría:  </span>{category ? category.nombre : "Cargando..."}</p>
                <p className={`income-card__state--${stateClassName}`}><span>Estado: </span>{state}</p>
            </div>
